...
final class FrameProvider {
	...
	monthHeaderHeight: CGFloat)
  {
	...
	daySize = CGSize(width: points, height: content.dayHeight ?? points)
	weekDaySize = CGSize(width: points, height: content.daysOfTheWeekHeight ?? points)
	...
	-> CGRect
  {
	...
	return CGRect(origin: CGPoint(x: x, y: y), size: weekDaySize)
  }
  ...
  func frameOfDay(_ day: Day, inMonthWithOrigin monthOrigin: CGPoint) -> CGRect {
	...
	let y = monthOrigin.y +
	  ...
	  content.monthDayInsets.top +
		(monthsLayout.pinDaysOfWeekToTop ? 0 : (weekDaySize.height + content.daysOfTheWeekBottomMargin)) +
	...
	-> CGRect
  {
	...
	return CGRect(origin: CGPoint(x: x, y: yContentOffset), size: weekDaySize)
  }

  func frameOfPinnedDaysOfWeekRowBackground(yContentOffset: CGFloat) -> CGRect {
	CGRect(x: layoutMargins.leading, y: yContentOffset, width: monthWidth, height: weekDaySize.height)
  }
	...
	-> CGRect
  {
	CGRect(
	  ...
	  y: yContentOffset + weekDaySize.height - separatorHeight,
	  width: monthWidth,
	...
	separatorHeight: CGFloat) -> CGRect
  {
	CGRect(
	  x: monthOrigin.x,
	  y: monthOrigin.y + monthHeaderHeight + content.monthDayInsets.top + weekDaySize.height -
		separatorHeight,
	  width: monthWidth,
	  height: separatorHeight)
  }
